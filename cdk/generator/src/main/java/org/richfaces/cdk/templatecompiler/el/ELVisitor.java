...
import org.richfaces.cdk.templatecompiler.el.node.AstMethodParametersTreeNode;
...
public final class ELVisitor implements TypedTemplateStatement {
    ...
    public ITreeNode determineNodeType(Node child) throws ParsingException {
        ...
        if (child instanceof AstIdentifier) {
        ...
        } else if (child instanceof AstMethodArguments) {
            treeNode = new AstMethodParametersTreeNode(child);
        } else if (child instanceof AstDotSuffix) {
            if (isCompositeComponentAttributesMap(child)) {
                ...
                treeNode = new AstIdentifierTreeNode(child);
            } else if (hasMethodParameters(child)) {
    ...
    private boolean hasMethodParameters(Node node) {
        return node.jjtGetNumChildren() == 1 && node.jjtGetChild(0) instanceof AstMethodArguments;
    }
