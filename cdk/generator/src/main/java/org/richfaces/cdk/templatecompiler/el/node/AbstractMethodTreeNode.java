...
public abstract class AbstractMethodTreeNode extends AbstractTreeNode {
    ...
    protected void visitMethod(StringBuilder sb, ELVisitor visitor, String methodName) throws ParsingException {
        ...
        if (childrenCount == 1 && getNode().jjtGetChild(0) instanceof AstMethodArguments) {
            String childOutput = getChildOutput(0, visitor);
            sb.append(childOutput);
            
            MethodTypesWrapper wrapper = (MethodTypesWrapper) visitor.getType();
            argumentTypes.addAll(wrapper.getTypes());
        }
        else {
